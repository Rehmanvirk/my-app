...
//    }
        ...
        if (state.error != null && state.error != "null")
            Toast.makeText(context, state.error, Toast.LENGTH_LONG).show()
        DynamicFormScreen(navController, state, title, fields = state.fields, onCancel = {
            navController.popBackStack()
        }) { values ->
            ...
            fields.forEach { f ->
                put(
                    ...
                    when (f.dataType) {
                        ...
                        "phone" -> FieldValue.Text("")
                        "datetime" -> FieldValue.DateTime(null)
                        "date" -> FieldValue.Date(null)
                        "picklist" -> FieldValue.Pick(null)
                        "boolean" -> FieldValue.Select(null)
...
//        Text("Dynamic Form", style = MaterialTheme.typography.titleLarge)
            ...
            fields.forEach { field ->
                ...
                when (field.dataType) {
                    "text" -> {
                        val current =
                            state[field.apiName] as? FieldValue.Text ?: FieldValue.Text("")
                        TextInputField(
