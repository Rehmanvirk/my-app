...
import com.app.calllogs.ui.tasks.TaskCard
...
//    }
    ...
    Column(
        ...
        TopBarWithBack(title = "Meetings",onCreateLeade = {
...
//        )
        ...
        if (!state.isLoading && state.allMeetings.isEmpty()) {
            Box(
        ...
        LazyColumn(
            ...
            if (state.isLoading) {
                ...
                items(state.allMeetings) { lead ->
                    TaskCard(lead = lead, onClick = {
                        val idToPass = lead.id
                        navController.navigate("EditMeetingScreen/$idToPass") {
                            popUpTo("EditMeetingScreen"){
                                this.inclusive = true
                            }
                        }
                        /* open lead details */ })
...
@Composable
private fun TaskCard(lead: Meetings, onClick: () -> Unit) {
    Surface(
        onClick = onClick,
