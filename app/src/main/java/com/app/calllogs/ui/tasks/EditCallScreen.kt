...
fun EditCallScreen(
    ...
    var callDuration by rememberSaveable  { mutableStateOf("") }
    var callPurpose by rememberSaveable  { mutableStateOf("") }
    var callAgenda by rememberSaveable  { mutableStateOf("") }
    var description by rememberSaveable  { mutableStateOf("") }

    LaunchedEffect(meeting.callsDet) {
        title = meeting.callsDet?.raw?.subject.orEmpty()
        contactName = meeting.callsDet?.agent?:""
        callType =  meeting.callsDet?.raw?.callType?:""
        callPurpose =  meeting.callsDet?.raw?.callPurpose?:""
        description =  meeting.callsDet?.raw?.description?:""
        callAgenda = meeting.callsDet?.raw?.callAgenda?:""
        startTime = meeting.callsDet?.raw?.callStartTime.orEmpty()
        callDuration = meeting.callsDet?.raw?.callDuration.orEmpty()
    }
...
//        CircularProgressIndicator(modifier = Modifier.wrapContentSize())
        ...
        Column(
            ...
            Column(
                ...
                // Name
                Text(
                    text = meeting.callsDet?.raw?.subject?:"",
                    fontSize = 18.sp,
...
//                Spacer(modifier = Modifier.height(32.dp))
                ...
                Button(
                    onClick = {
                        viewModel.updateCall(id,title,description)
                    },
